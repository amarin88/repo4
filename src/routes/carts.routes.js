...
router.post("/", async (req, res) => {
    try {
        ...
        res.status(201).json(cart); // Responde con el carrito creado y el código de estado 201 (creado)
    } catch (error) {
...
router.post("/:cid/product/:pid", async(req, res) =>{
    try {
        ...
        res.status(201).json(cart); // Responde con el carrito actualizado y el código de estado 201 (creado)
    } catch (error) {
        console.log(error); // Registra cualquier error en la consola
        return res.json({
            status: error.status || 500, // Devuelve el código de estado del error o 500 si no está definido
...
router.get("/:cid", async (req, res) => {
    try {
        ...
        res.status(200).json(cart); // Responde con los productos del carrito y el código de estado 200 (éxito)
    } catch (error) {
        console.log(error); // Registra cualquier error en la consola
        return res.json({
            status: error.status || 500, // Devuelve el código de estado del error o 500 si no está definido
